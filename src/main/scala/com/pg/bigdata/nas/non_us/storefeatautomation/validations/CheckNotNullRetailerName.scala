...
import com.pg.bigdata.nas.non_us.storefeatautomation.datamodel.Columns.{cnt, id, pos_cust_id, pos_store_id}
...
class CheckNonUinquePosStoreId(storeFeatDf: DataFrame) extends Validation {
  ...
  override def prepareData(implicit spark: SparkSession, logger: UtilLogger): DataFrame = {
    storeFeatDf.groupBy(pos_cust_id, pos_store_id).agg(count(id).alias(cnt)).where(cnt > 1)
  }
